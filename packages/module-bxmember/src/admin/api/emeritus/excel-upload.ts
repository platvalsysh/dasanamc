...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  if (!auth.checkPermissions(["bxmember.emeritus.excel.upload"])) {
  ...
  try {
    ...
    // Emeritus: seq, name_kor, email, cellphone_number => 4 columns
    ...
    const s3Key = `bxmember/emeritus/${timestamp}_${crypto.randomUUID()}.${extension}`;
    ...
    // Pre-process data
    const emeritussToProcess = dataRows.map(row => {
    ...
    await prisma.$transaction(async (tx) => {
      ...
      const providedSeqs = emeritussToProcess
          .map(p => p.seq)
      ...
      if (providedSeqs.length > 0) {
          const existingRecords = await tx.bxemeritus.findMany({
      ...
      // B. Max Seq
      const last = await tx.bxemeritus.findFirst({ orderBy: { seq: 'desc' } });
      let currentMaxSeq = last?.seq || 0n;

      // C. Process
      for (const memberData of emeritussToProcess) {
        const { seq, ...data } = memberData;

        if (seq) {
          if (existingSeqSet.has(seq)) {
             // Update
             await tx.bxemeritus.update({
               where: { seq },
               data: data
             });
          } else {
             // Insert specific
             await tx.bxemeritus.create({
               data: {
                 ...data,
                 seq,
               }
