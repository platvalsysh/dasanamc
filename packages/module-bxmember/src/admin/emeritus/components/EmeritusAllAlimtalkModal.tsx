...
interface EmeritusAllAlimtalkModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  smsTestConfig: { mode: "test", testNumber: string } | { mode: "production" };
}

export function EmeritusAllAlimtalkModal({ open, onOpenChange, smsTestConfig }: EmeritusAllAlimtalkModalProps) {
  ...
  useEffect(() => {
    if (open) {
      ...
      // Fetch all emerituss preview
      setLoadingPreview(true);
      fetch("/admin/api/bxmember/emeritus/all", { method: "POST" })
        .then(res => res.json())
        .then(res => {
          if (res.success) {
            const profs = res.data.emerituss;
