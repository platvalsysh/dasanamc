...
interface EmeritusAllSmsModalProps {
  open: boolean;
  onOpenChange: (open: boolean) => void;
  smsTestConfig: { mode: "test", testNumber: string } | { mode: "production" };
}

export function EmeritusAllSmsModal({ open, onOpenChange, smsTestConfig }: EmeritusAllSmsModalProps) {
  const [message, setMessage] = useState("");
  const [sending, setSending] = useState(false);
  
  const [emerituss, setEmerituss] = useState<any[]>([]);
  ...
  const fetchPreview = async () => {
    ...
    try {
      const res = await fetch("/admin/api/bxmember/emeritus/all", { method: "POST" });
      const data = await res.json();
      if (data.success) {
        const profs = data.data.emerituss;
        setEmerituss(profs);
