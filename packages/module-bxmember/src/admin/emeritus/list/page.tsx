...
import { prisma, type Prisma, type bxemeritus } from "@repo/database";
...
import { CreateEmeritusDialog } from "./components/CreateEmeritusDialog";
import { EditEmeritusDialog } from "./components/EditEmeritusDialog";
...
import { EmeritusAllSmsModal } from "../components/EmeritusAllSmsModal";
import { EmeritusAllEmailModal } from "../components/EmeritusAllEmailModal";
import { EmeritusAllAlimtalkModal } from "../components/EmeritusAllAlimtalkModal";
import { z } from "zod";
...
export async function loader({ request, context }: LoaderFunctionArgs) {
  ...
  // Check permission - adjusting to use specific emeritus permission if mandated, or generic list if not defined yet.
  // Plan said 'bxemeritus.list'. I will assume I'll add this.
    if (!auth.checkPermissions(["bxmember.emeritus.list"])) {
  ...
  const [total, data] = await Promise.all([
    prisma.bxemeritus.count({ where }),
    prisma.bxemeritus.findMany({
      where,
      skip,
      take: limit,
      orderBy: { seq: "asc" },
...
export async function action({ request, context }: ActionFunctionArgs) {
  ...
  if (intent === "create") {
    if (!auth.checkPermissions(["bxmember.emeritus.create"])) return response({ success: false, error: "Permission denied" }, { status: 403 });
    const last = await prisma.bxemeritus.findFirst({ orderBy: { seq: "desc" } });
    const seq = (last?.seq || 0n) + 1n;
    const data = EmeritusSchema.parse(Object.fromEntries(formData));
    await prisma.bxemeritus.create({ data: { seq, ...data } });
    return response({ success: true });
  }

  if (intent === "update") {
     if (!auth.checkPermissions(["bxmember.emeritus.edit"])) return response({ success: false, error: "Permission denied" }, { status: 403 });
    const seq = BigInt(formData.get("seq") as string);
    const data = EmeritusSchema.parse(Object.fromEntries(formData));
    await prisma.bxemeritus.update({ where: { seq }, data });
    return response({ success: true });
  }

  if (intent === "delete") {
     if (!auth.checkPermissions(["bxmember.emeritus.delete"])) return response({ success: false, error: "Permission denied" }, { status: 403 });
    const seq = BigInt(formData.get("seq") as string);
    await prisma.bxemeritus.delete({ where: { seq } });
    return response({ success: true });
  }
...
export default function EmeritusListPage() {
  ...
  const [selectedEmeritus, setSelectedEmeritus] = useState<bxemeritus | null>(null);
  ...
  const handleEdit = (emeritus: bxemeritus) => {
    setSelectedEmeritus(emeritus);
    setEditOpen(true);
  };
  ...
  return (
    <div className="p-2 md:p-6 space-y-6">
      <div className="flex flex-col gap-4 items-start">
        <h1 className="text-2xl font-bold text-gray-900">명예교수님</h1>
            ...
            <button
              ...
              onClick={() => {
                ...
                if (hasSearch) {
                  ...
                  window.open("/admin/api/bxmember/emeritus/excel-download", "_blank");
                }
              }}
              className="p-2 rounded-md transition-colors flex-1 md:flex-none flex items-center justify-center gap-1 text-sm font-medium bg-green-50 text-green-700 border border-green-200 hover:bg-green-100"
              title="엑셀 다운로드"
            >
      ...
      <CreateEmeritusDialog open={createOpen} onOpenChange={setCreateOpen} />
      <EditEmeritusDialog open={editOpen} onOpenChange={setEditOpen} emeritus={selectedEmeritus} />
      ...
      {/* New Group Modals for Emerituss */}
      <EmeritusAllSmsModal 
        open={groupSmsOpen}
        onOpenChange={setGroupSmsOpen}
        smsTestConfig={smsTestConfig}
      />
      <EmeritusAllEmailModal
        open={groupEmailOpen}
