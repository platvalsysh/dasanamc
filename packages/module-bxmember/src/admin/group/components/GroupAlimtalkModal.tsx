...
export function GroupAlimtalkModal({ open, onOpenChange, members, groupName, smsTestConfig }: GroupAlimtalkModalProps) {
    ...
    if (open) {
      ...
      for (const m of members) {
        ...
        if (isValidPhoneNumber(phone)) {
            valid.push({ name, phone: phone!, id: m.id });
        } else {
            invalid.push({ name, phone: phone || undefined, reason: phone ? "형식 오류" : "번호 없음", id: m.id });
        }
      }
  ...
  const executeSend = async () => {
    ...
    const recipients = validRecipients.map(r => ({ phone: r.phone, name: r.name }));
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-7xl max-h-[95vh] overflow-y-auto w-[95vw]">
        <DialogHeader><DialogTitle>그룹 알림톡 전송: {groupName}</DialogTitle></DialogHeader>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 py-4">
          ...
          <div className="border rounded-lg p-4 bg-gray-50 flex flex-col h-full">
            <h3 className="font-semibold mb-2 flex items-center gap-2">수신자 목록 ({groupName})</h3>
            <Tabs defaultValue="valid" className="flex flex-col h-full">
                    ...
                    {validRecipients.length > 0 ? (
                        <ul className="text-sm space-y-1">
                            {validRecipients.map((m, i) => (
                                <li key={m.id} className="flex justify-between border-b pb-1 last:border-0 text-blue-700">
                                    <span>{m.name}</span>
                ...
                <TabsContent value="invalid" className="flex-1 overflow-y-auto min-h-75 max-h-125 bg-white border rounded p-2 mt-2">
                    {invalidRecipients.length > 0 ? <ul className="text-sm space-y-1">{invalidRecipients.map((m, i) => <li key={m.id} className="flex justify-between border-b pb-1 last:border-0 text-gray-400"><span>{m.name}</span><span>{m.reason}</span></li>)}</ul> : <p className="text-gray-500 text-center py-10">제외 없음</p>}
                </TabsContent>
            </Tabs>
          </div>
        </div>
        ...
        <Dialog open={confirmOpen} onOpenChange={setConfirmOpen}>
            <DialogContent><DialogHeader><DialogTitle>알림톡 발송 확인</DialogTitle></DialogHeader>
                <div className="py-2"><p>해당 그룹의 <strong>{validRecipients.length}명</strong>에게 알림톡을 전송하시겠습니까?</p></div>
