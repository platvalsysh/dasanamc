...
interface SelectedSmsModalProps {
  open: boolean;
...
export function SelectedSmsModal({ open, onOpenChange, recipients, smsTestConfig }: SelectedSmsModalProps) {
  ...
  // Reset state on open
  // Reset state on open & calculate recipients
  useEffect(() => {
    if (open) {
      setMessage("");
      ...
      for (const r of recipients) {
          if (r.cellphone_number && isValidPhoneNumber(r.cellphone_number)) {
              ...
              invalid.push({
                  name_kor: r.name_kor,
                  cellphone_number: r.cellphone_number,
                  reason: r.cellphone_number ? "형식 오류" : "번호 없음"
              });
          }
      }
  ...
  const executeSend = async () => {
    setConfirmOpen(false);
    setSending(true);
    const payloadRecipients = validRecipients.map(r => ({
  ...
  return (
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="max-w-4xl max-h-[95vh] overflow-y-auto w-[95vw]">
        <DialogHeader>
          <DialogTitle>선택 문자 전송</DialogTitle>
        </DialogHeader>
                    ...
                    {validRecipients.length > 0 ? (
                        <ul className="text-sm space-y-1">
                            {validRecipients.map((m, i) => (
                                <li key={`valid-${i}-${m.cellphone_number}`} className="flex justify-between border-b pb-1 last:border-0">
