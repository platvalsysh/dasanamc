...
interface PositionTransferListProps {
    options: PositionOption[];
    selectedIds: string[];
    onChange: (ids: string[]) => void;
}

export function PositionTransferList({
    options,
    selectedIds,
    onChange,
}: PositionTransferListProps) {
    ...
    const moveToRight = () => {
        ...
        onChange([...selectedIds, ...leftSelection]);
        setLeftSelection([]); // Clear selection after move
    };

    const moveToLeft = () => {
        if (rightSelection.length === 0) return;
        onChange(selectedIds.filter(id => !rightSelection.includes(id)));
        setRightSelection([]); // Clear selection after move
    };

    const moveAllToRight = () => {
        if (availableOptions.length === 0) return;
        onChange([...selectedIds, ...availableOptions.map(o => o.id)]);
        setLeftSelection([]);
    };
    ...
    return (
        <div className="grid grid-cols-[1fr_auto_1fr] gap-4 h-[400px] items-start">
            ...
            <div className="flex flex-col h-full space-y-2">
                <Label>전체 직책 ({availableOptions.length})</Label>
                <select
                    multiple
                ...
                >
                    {availableOptions.map(opt => (
                        <option key={opt.id} value={opt.id} className="py-1 px-2 cursor-pointer hover:bg-gray-100">
                            {opt.label}
                        </option>
                    ))}
                </select>
            ...
            {/* Right List: Selected */}
            <div className="flex flex-col h-full space-y-2">
                <Label>선택된 직책 ({selectedOptions.length})</Label>
                <select
                    multiple
                ...
                >
                    {selectedOptions.map(opt => (
                        <option key={opt.id} value={opt.id} className="py-1 px-2 cursor-pointer hover:bg-indigo-100">
                            {opt.label}
                        </option>
                    ))}
                </select>
