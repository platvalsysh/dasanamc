...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  try {
      ...
      await sender.sendLMS({ to: testPhone, title, text });
      return { success: true, message: `LMS sent to ${testPhone} (Console)` };
  } catch (e: any) {
      return { error: e.message };
  }
}

export default function ConsoleLmsTestPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
         <div>
             <h2 className="text-2xl font-bold tracking-tight text-foreground">Send LMS</h2>
             <p className="text-muted-foreground">Send a test LMS via Console.</p>
         </div>
             ...
             <CardContent className="pt-6">
                <Form method="post" className="space-y-4">
                    ...
                    <div className="space-y-2">
                        ...
                        <Textarea name="text" placeholder="Type your long message..." required className="h-32 text-foreground bg-transparent" />
                    </div>
                    <Button type="submit" className="w-full" disabled={isSending}>
                        {isSending ? "Sending..." : <><Send className="w-4 h-4 mr-2"/> Send LMS</>}
                    </Button>
                </Form>
             </CardContent>
         </Card>
