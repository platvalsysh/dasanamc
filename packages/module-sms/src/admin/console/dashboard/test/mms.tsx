...
export async function action({ request, params }: ActionFunctionArgs) {
  ...
  try {
      ...
      await sender.sendMMS({ to: testPhone, title, text, imageUrls: [] });
      return { success: true, message: `MMS sent to ${testPhone} (Console)` };
  } catch (e) {
      return { error: getErrorMessage(e) };
  }
}

export default function ConsoleMmsTestPage() {
  ...
  return (
    <div className="space-y-6 max-w-2xl">
         <div>
             ...
             <p className="text-muted-foreground">Send a test MMS via Console.</p>
         </div>
